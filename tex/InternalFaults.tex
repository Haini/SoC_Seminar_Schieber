...
With \gls{TMR} the whole resilient circuit needs to be replicated and a voter design needs to be added.
In comparison, with \gls{TR}, only sequential resources of a circuit need to be duplicated.
Forward \gls{TR} reduces the delay introduced by \gls{TR} while still providing the same granularity for localizing faults. 
